...
type ReactivateOfficePageProps = {
    params: Promise<{
        id: string;
    }>;
...
async function reactivateOfficeAction(formData: FormData) {
    "use server";
    ...
    if (!user) {
        redirect(`/login?next=/authority/registry/${officeId}/reactivate`);
    }
    ...
    if (!profile?.is_verified) {
        redirect(`/login?next=/authority/registry/${officeId}/reactivate&verify=1`);
    }
    ...
    const reason =
        ...
        "Reactivated by central authority. Office is active again but still pending account verification.";

    const { error } = await supabase.rpc("reactivate_authority_office", {
        p_office_id: officeId,
        p_note: reason,
    });

    if (error) {
        console.error("Reactivate office error:", error.message);
        redirect(`/authority/registry/${officeId}/reactivate?error=1`);
    }

    redirect(`/authority/registry/${officeId}`);
}

export default async function ReactivateOfficePage({
    params,
    searchParams,
}: ReactivateOfficePageProps) {
    ...
    if (!user) {
        redirect(`/login?next=/authority/registry/${id}/reactivate`);
    }
    ...
    if (!profile?.is_verified) {
        redirect(`/login?next=/authority/registry/${id}/reactivate&verify=1`);
    }
                        ...
                        <section className={styles.detailHero}>
                            <div className={styles.detailTop}>
                                <div className={styles.detailMetaBlock}>
                                    <p className={styles.eyebrow}>Reactivate service desk</p>
                                    <h1 className={styles.title}>{office.office_name}</h1>
                                    <p className={styles.subtitle}>
                                        Reactivate this service desk so it can be used again for
                                        future complaint routing. Reactivation does not verify the
                                        office account automatically.
                                    </p>
                        ...
                        <section className={styles.detailGrid}>
                            <div className={styles.leftColumn}>
                                <article className={styles.panel}>
                                    <h2 className={styles.panelTitle}>Reactivate confirmation</h2>

                                    <div className={styles.panelBody}>
                                        {hasError ? (
                                            <div className={styles.alertBox}>
                                                Failed to reactivate this service desk. Please check the
                                                database function and your authority permission, then
                                                try again.
                                            </div>
                                        ) : null}
                                        ...
                                        {alreadyActive ? (
                                            <div className={styles.infoBox}>
                                                <p className={styles.kvLabel}>Already active</p>
                                                <p className={styles.kvValue}>
                                                    This service desk is already active. No reactivation
                                                    action is needed.
                                                </p>
                                            </div>
                                        ) : (
                                            <div className={styles.infoBox}>
                                                <p className={styles.kvLabel}>Reactivation rule</p>
                                                <p className={styles.kvValue}>
                                                    This will make the service desk active again, but it
                                                    will remain pending setup until a real authority
                                            ...
                                            >
                                                ...
                                                <div>
                                                    ...
                                                    <textarea
                                                        ...
                                                        defaultValue={`Reactivated by central authority. This office can receive new complaint routing again, but it remains pending account verification.`}
                                                    />
                                                </div>
